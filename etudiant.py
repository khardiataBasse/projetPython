...
from pythonProject.bd import getEtudiant,liste_classe, connexionDB
            ...
            try:
                  ...
                  if row!=None:
                      messagebox.showerror("Erreur", "ce mail exite deja.Essayer un autre mail")
                       ...
                       sql2 = "INSERT INTO Etudiant (matricule,classe, PersonID) VALUES(%s, %s, %s)"
                       val2 = (matricule.get(), classe.get(), idpers)
                       meconnecter.execute(sql2,val2)
                       mabase.commit()
                       messagebox.showinfo("information", "etudiant ajouter", parent=root)
    ...
    if modifier==True:
        if nom.get() != "" and prenom.get() != "" and sexe.get() != "" and statut.get() != "" and mail.get()!= "" and adresse.get() != "" and telephone.get() != "":
            try:
                  mabase=connexionDB()
                  meconnecter=mabase.cursor()
                  sql="update Personne p, Etudiant e set p.nom=%s,p.prenom=%s, p.date_naissance=%s," \
                        "p.sexe=%s,p.statut=%s,p.mail=%s,p.adresse=%s,p.tel=%s, e.classe=%s"
                  val=(nom.get(),prenom.get(),datenaissance.get(),sexe.get(),statut.get(),mail.get(),adresse.get(),telephone.get(),classe.get())
...
def supprimerEtudiant():
    ...
    if int(id)>0:
      ...
      mabase.commit()
      table.delete(id)
      messagebox.showinfo("Info", "Supression reussi", parent=root)
      root.mainloop()
    else:
...
def selectionner(event):
    id = table.selection()[0]
    if int(id)>0:
        matricule.set(table.item(id, "values")[1])
        ...
        classe.set(table.item(id, "values")[10])
def modifierFalse():
    global modifier
    modifier=False
...
#ajouter le titre
lbltitre=Label(root,borderwidth=3,relief=SUNKEN, text="Ajouter un etudiant",
...
#Donnees
#Matricule
lblmatricule= Label (root, text="Matricule :", font=("Arial,18"), bg="#091821",
                        fg="white")
lblmatricule.place(x=100,y=100,width=150)
textmatricule=Entry(root, bd=4,font=("Arial",14), textvariable=matricule)
textmatricule.place(x=230,y=100,width=150)
...
lblclasse.place(x=630,y=300,width=150)
listeClasse=ttk.Combobox(root, font=("Arial",14), textvariable=classe)
listeClasse['values']=liste_classe()
listeClasse.place(x=760,y=300,width=130)
lblmessage=Label(root, text="" , font=("Arial,18"),bg="#091821",fg="white")
lblmessage.place(x=630, y=320)

#enregistrer
btnenregistrer=Button(root, text="Sauvegarder", font=("Arial",16), bg="#D2691E", fg="white",command=ajouter_etudiant)
btnenregistrer.place(x=500,y=350,width=200)
#modifier
btnmodifier=Button(root, text="selectionner", font=("Arial",16), bg="#D2691E", fg="white",command=modifier_etudiant)
btnmodifier.place(x=800,y=350,width=200)
#supprimer
btnsupprimer=Button(root, text="Supprimer", font=("Arial",16), bg="#D2691E", fg="white",command=supprimerEtudiant)
...
table.heading(2,text="Matricule")
